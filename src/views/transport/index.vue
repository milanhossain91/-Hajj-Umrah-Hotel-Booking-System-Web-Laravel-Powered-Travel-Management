<template>
  <MainLayout>
      ...
      <div class="relative">
        ...
        <table class="min-w-full table-auto shadow-lg rounded-lg overflow-hidden">
          ...
          <tbody>
            ...
            <!-- Empty state -->
            <tr v-if="!isTransportLoading && transports.length === 0">
    ...
    <a-pagination class="p-4" v-model:current="currPage" v-model:page-size="itemsPerPage" :total="totalItems"
      ...
      @change="getTransports($event)" @showSizeChange="(current, pageSize) => (itemsPerPage = pageSize)" />
      ...
      <form @submit.prevent="createQuote">
        <div class="grid grid-cols-1 gap-4">
          ...
          <div>
            ...
            <a-input id="name" v-model:value="newTransport.name" type="text" class="w-full p-2 border rounded-md"
              required />
      ...
      <form @submit.prevent="updateTransport">
        <div class="grid grid-cols-1 gap-4">
          ...
          <div>
            ...
            <a-input id="update_name" v-model:value="currentTransport.name" type="text"
...
// Fetch transports
const getTransports = async (page = 1) => {
  isTransportLoading.value = true;
  try {
    // Build the base URL
    let url = `${apiBase}/transports?page=${page}&limit=${itemsPerPage.value}`;
...
// Form data for transport creation
const newTransport = ref({
  name: ''
});

// Current transport for updating
...
const createQuote = async () => {
  ...
  try {
    const res = await axios.post(`${apiBase}/transports`, newTransport.value, getToken());
    if (res.data) {
      createModalVisible.value = false;
      newTransport.value.name = ''; // Reset form
      await getTransports(); // Refresh the list
      showNotification('success', 'Success', 'Transport created successfully');
    }
  } catch (error) {
...
const updateTransport = async () => {
  ...
  try {
    const res = await axios.put(`${apiBase}/transports/${currentTransport.value.id}`, currentTransport.value, getToken());
    if (res.data) {
...
const deleteTransport = async (id) => {
  ...
  try {
    await axios.delete(`${apiBase}/transports/${id}`, getToken());
    await getTransports(); // Refresh the list
    showNotification('success', 'Success', 'Transport deleted successfully');
  } catch (error) {
    console.error(error);
    showNotification('error', 'Failed', 'Failed to delete transport');
  } finally {

<template>
  <MainLayout>
    <div class="overflow-x-auto bg-white rounded-lg shadow-md">
      <!-- Customer List -->
      <p class="text-2xl font-bold text-center py-8">Customer List</p>
      ...
      <div class="flex items-center justify-between m-4">
        ...
        <button @click="showCreateModal"
          ...
          <Icon icon="mdi:plus-circle" class="mr-2" /> Add New Customer
        </button>
      </div>
      ...
      <table class="min-w-full table-auto shadow-lg rounded-lg overflow-hidden">
        ...
        <tbody>
          ...
          <tr v-for="(customer, index) in customers" :key="index"
            ...
            <td class="px-6 py-4 text-sm text-gray-800">{{ customer?.location || '-'}}</td>
        ...
        <div class="grid grid-cols-1 gap-4">
          ...
          <div>
            <label for="location" class="block text-gray-700">Location</label>
             <input id="location" v-model="newCustomer.location" type="text" class="w-full p-2 border rounded-md" required />
          </div>
        </div>
      </form>
    </a-modal>
    ...
    <a-modal v-model:open="updateModalVisible" title="Update Customer" @ok="updateCustomer">
      <form @submit.prevent="updateCustomer">
        <div class="grid grid-cols-1 gap-4">
          ...
          <div>
            <label for="location" class="block text-gray-700">Location</label>
            <input id="location" v-model="currentCustomer.location" type="text" class="w-full p-2 border rounded-md"
              required />
          </div>
        </div>
      </form>
...
const getCustomers = async (page = 1) => {
  ...
  try {
    let url = `${apiBase}/customers?page=${page}&limit=${itemsPerPage.value}`;
...
const showCreateModal = () => {
  newCustomer.value = { name: '', email: '', phone: '', location: '' };
  createErrors.value = {};
...
const createCustomer = async () => {
  createErrors.value = {};
  try {
    const res = await axios.post(`${apiBase}/customers`, newCustomer.value, getToken());
    if (res.data) {
...
const updateCustomer = async () => {
  updateErrors.value = {};
  try {
    const res = await axios.put(`${apiBase}/customers/${currentCustomer.value.id}`, currentCustomer.value, getToken());
    if (res.data) {
...
const deleteCustomer = async (id) => {
  ...
  try {
    const response =  await axios.delete(`${apiBase}/customers/${id}`, getToken());
